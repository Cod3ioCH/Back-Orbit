...
export function RestorePage(){
  ...
  return <div className="space-y-6"><PageHeader title="Restore" description="Preview impact first. Back-Orbit only enables actions that the selected snapshot can perform safely."/>
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,.6fr)]"><div className="space-y-5">
      <Card><CardHeader><CardTitle>1. Choose a recovery point</CardTitle><CardDescription>Only verified snapshots are available.</CardDescription></CardHeader><CardContent><Label id="snapshot-label">Snapshot</Label><Select<string> value={snapshotId||null} onValueChange={(value)=>{setSnapshotId(value??"");preview.reset()}}><SelectTrigger className="mt-2 w-full" aria-labelledby="snapshot-label"><SelectValue>{(value)=>{const selected=snapshots.find(snapshot=>snapshot.id===value);return selected?`${selected.projectName} · ${new Date(selected.createdAt).toLocaleString()} · ${selected.repositoryName}`:"Select a snapshot…"}}</SelectValue></SelectTrigger><SelectContent>{snapshots.map(snapshot=><SelectItem key={snapshot.id} value={snapshot.id}><span className="flex min-w-0 flex-col"><span className="truncate font-medium">{snapshot.projectName}</span><span className="truncate text-xs text-muted-foreground">{new Date(snapshot.createdAt).toLocaleString()} · {snapshot.repositoryName}</span></span></SelectItem>)}</SelectContent></Select></CardContent></Card>
